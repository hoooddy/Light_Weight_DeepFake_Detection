...
from efficientnet_elite import EfficientNetElite  # EfficientNet-eLite 사용
...
class EfficientViT(nn.Module): 
    def __init__(self, config, channels=512, selected_efficient_net=0):
        ...
        # EfficientNet-eLite 적용
        if selected_efficient_net == 0:
            try:
                self.efficient_net = EfficientNetElite.from_pretrained('efficientnet_elite_l9')  # EfficientNet-eLite L9
            except FileNotFoundError:
                print("EfficientNet-eLite L9 pretrained weights not found. Initializing randomly.")
                self.efficient_net = EfficientNetElite()  # 기본 초기화
        else:
            try:
                self.efficient_net = EfficientNetElite.from_pretrained('efficientnet_elite_l7')  # EfficientNet-eLite L7
            except FileNotFoundError:
                print("EfficientNet-eLite L7 pretrained weights not found. Initializing randomly.")
                self.efficient_net = EfficientNetElite()  # 기본 초기화
    ...
    def forward(self, img, mask=None):
        p = self.patch_size
        x = self.efficient_net.extract_features(img)  # EfficientNet-eLite 특징 추출
