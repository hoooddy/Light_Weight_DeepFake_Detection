...
if __name__ == "__main__":
    ...
    TEACHER_MODEL_PATH = "result/2024-12-12_23-01-31/efficientnetB0"
    model_t = EfficientViT(config=config, channels=channels, selected_efficient_net = opt.efficient_net)
    ...
    for t in tqdm(range(starting_epoch, opt.num_epochs + 1)):
        ...
        if val_f1_score == max(val_f1_score_list):
            torch.save(model_s.state_dict(), os.path.join(RESULT_PATH,  "efficientnetB" + str(opt.efficient_net)))
